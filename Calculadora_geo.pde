...
public static class Calculadora_geo {
  ...
  public static float[] puntoTriangular (float px, float py, float x1, float y1, float x2, float y2) {
    if (x1 == x2)
      return new float[] {x1, py};
    
    float m = calcularPendiente(x1, y1, x2, y2);
    
    if (m == 0)    //Si la recta es paralela al eje x
      return new float[] {px, y1};
    
    float x = valorAbs(((py - y1) / m) - x1);
    return new float[] {x, py};
  }
  ...
  public static int[][] calcularAreaCuadrada (int x1, int y1, int x2, int y2, int rad) {
    ...
    int tx = Math.round(rad * valor);
    int ty = Math.round(rad * valorAbs(m) * valor);
